...
import namedEntity.heuristic.DictionaryHeuristic;
    ...
    public static Heuristic getHeuristic(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-h") && i + 1 < args.length) {
                ...
                else if (heuristicArg.equals("dict")) {
                    heuristic = new DictionaryHeuristic();
                }
                else {
        ...
        for(String category : categoriesMutable.keySet()){
            for(NamedEntity entity : globalEntityMap.values()){
                if(entity.getCategory().equals(category)){
                    categoriesMutable.put(category, 
                        (categoriesMutable.get(category) + 
                        entity.getFrequency()));
                }
            }
        }
